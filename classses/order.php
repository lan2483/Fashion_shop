    ...
    class order
    {
        ...
        public function insertOder($userId,$date){
			...
			if($getProduct){
                while($result= $getProduct->fetch_assoc()){
                    ...
                    $order_time = $date;
                    $query_order= "INSERT INTO tbl_order(productId,size,price,image,quantity,
                                    thanhtien,userId, status,order_time,recieve_time) VALUES('$productId',
        ...
        public function getOrderHistory($userId,$status,$date){
         ...
         public function recieve_Order($orderId,$status,$userId,$date_current){
            $query= "UPDATE tbl_order SET status = '$status', recieve_time = '$date_current' 
                    WHERE orderId IN $orderId AND userId='$userId'";
            $result = $this->db->update($query);
         }
        ...
        public function admin_getOrder_waiting($userId,$status,$date){
            $query = "SELECT od.* , pd.productName ,username,us.userId , 
                    name,us.diaChi,us.name,pd.brandId,pd.typeProductId
            FROM tbl_order AS od
            INNER JOIN tbl_product AS pd ON od.productId =pd.productId
            INNER JOIN tbl_uer AS us ON od.userId =us.userId
            ...
            if($type==1){
                ...
                if($result_SQL)
                    while($product= $result_SQL->fetch_assoc()){
                        ...
                        $query= "UPDATE tbl_product SET quantity = quantity -'$quantity'   
