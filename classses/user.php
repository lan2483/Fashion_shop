...
class user
{
    ...
    public function insert_comments()
    {
        ...
        if ($tenbinhluan == "" || $comment == "" || $comment == ' ') {
            $alert = "<span class='fix_bug'>Enter the full information</span>";
        } else {
            ...
            if ($result) {
                $alert = "<span class='fix'>Thank you for comment!</span>";
                return $alert;
            } else {
                $alert = "<span class='fix_bug'> Comments failed !</span>";
                return $alert;
            }
        }
    }
    ...
    public function insert_rep_comments()
    {
        ...
        if ($rep_comment == "") {
            ...
            $result = $this->db->insert($query);
            if ($result) {
                $alert = "<span class='fix'>Successful reflection !</span>";
                return $alert;
            } else {
    ...
    public function insert_user($data)
    {
        ...
        if (
            ...
            $alert = "<span class='error'>Enter the full information</span>";
            return $alert;
        } else {
            if ($password != $relyPassword) {
                $alert = "<span class='error'> Import password does not match </span>";
                return $alert;
            } else {
                ...
                if ($result_check) {
                    $alert = "<span class='error'> Your username has existed </span>";
                    return $alert;
                } else {
                    $check_email = "SELECT * FROM tbl_uer WHERE email='$email'  LIMIT 1";
                    $result_mail = $this->db->select($check_email);
                    if ($result_mail) {
                        $alert = "<span class='error'> Your email has been registered </span>";
                        return $alert;
                    } else {
                        ...
                        $result = $this->db->insert($query);
                        if ($result) {
                            $alert = "<span class='success'> Sign Up Success  </span>";
                            return $alert;
                        } else {
    ...
    public function login_user($data)
    {
        ...
        if ($username == "" ||  $password == "") {
            ...
            if ($result) {
                ...
                $alert = "<span class='error'> Wrong account or password </span>";
                return $alert;
            }
        }
    }
    ...
    public function showCauHoiBiMat($username, $cauHoiBiMat)
    {
        ...
        if ($cauHoiBiMat == "" || $username == "") {
            $alert = "<span class='error'>You have not entered the question yet</span>";
            return $alert;
        } else {
    ...
    public function updatePass($data)
    {
        ...
        if ($password == "" ||  $relyPassword == "") {
            ...
            if ($password != $relyPassword) {
                $alert = "<span class='success'> Import password does not match </span>";
                return $alert;
            } else {
    ...
    public function updatedoimatkhau($matkhaumoi)
    {
        ...
        if ($sql_update == true) {
            $alert = '<span class="Update_pass">Change password successfully !</span>';
            return $alert;
        }
    }
}
